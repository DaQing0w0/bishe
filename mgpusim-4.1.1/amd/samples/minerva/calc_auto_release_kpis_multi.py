...
def compute_kpis_for_files(auto_files: List[AutoFileData], page_size: int) -> Dict[str, object]:
    ...
    for af in auto_files:
        for r in af.rows:
            ...
            if r.would_release is True:
                would_cnt += 1
            if r.did_release is not None:
                has_did_release_field = True
                if r.did_release is True:
                    did_cnt += 1

            if r.post_threshold_access is not None:
                post_den += 1
                if r.post_threshold_access > 0:
                    post_nonzero += 1
...
def print_table(results: Dict[str, Dict[str, object]]) -> None:
    headers = [
        ...
        "avg_resident_bytes",
        "peak_resident_bytes",
        "execution_ratio_did_over_would",
        "post_threshold_ratio",
        "avg_epoch_runtime",
        "avg_page_lifetime",
    ]
        ...
        for mode in ("dry_run", "enforce"):
            ...
            rows.append(
                [
                    ...
                    fmt_val(kpis.get("execution_ratio_did_over_would")),
                    fmt_val(kpis.get("post_threshold_ratio")),
